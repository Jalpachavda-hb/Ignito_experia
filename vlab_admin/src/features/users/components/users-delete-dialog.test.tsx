...
describe('UsersDeleteDialog', () => {
  ...
  it('renders the dialog with the correct title, description, input and buttons', async () => {
    ...
    const desc = getByText(
      new RegExp(`Are you sure you want to delete.*${MOCK_USER.FullName}.*`, 'i')
    )
    const nameInput = getByRole('textbox', { name: /Full Name/i })
  ...
  it('keeps the delete button disabled until the Full Name input is filled correctly', async () => {
    ...
    await userEvent.fill(nameInput, MOCK_USER.FullName)
  ...
  it('resets the Full Name input when the dialog is closed and reopened', async () => {
    function Harness() {
    ...
    const nameInput = getByRole('textbox', { name: /Full Name/i })
    await userEvent.fill(nameInput, MOCK_USER.FullName)
    await expect.element(nameInput).toHaveValue(MOCK_USER.FullName)
  ...
  it('calls delete mutation when deleted successfully', async () => {
    const onOpenChange = vi.fn()
    const { getByRole } = await render(
      <UsersDeleteDialog
        open
    ...
    expect(mockDeleteMutate.mock.calls[0][0]).toBe(MOCK_USER.UserId)
  })
})
