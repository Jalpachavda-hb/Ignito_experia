...
import { Shield, Users, Edit2, Trash2, Lock } from 'lucide-react'
...
export function RoleCard({ role, onEdit, onDelete }: RoleCardProps) {
  // Calculate permission summary across all modules
  let totalActive = 0
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium">
              {Intl.NumberFormat('en-US').format(role.userCount ?? 0)}
            </span>
            ...
            <TooltipTrigger asChild>
              <span tabIndex={0}>
                <Button
                  variant="outline"
                  size="icon"
                  className="text-muted-foreground border-border/50 cursor-not-allowed opacity-50"
                  disabled
                >
