...
export function LabActivity({ labs }: { labs: LabActivityType[] }) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'In Progress': return 'bg-blue-500/10 text-blue-600 border-blue-200 dark:border-blue-800 font-bold'
      case 'Stopped': return 'bg-amber-500/10 text-amber-600 border-amber-200 dark:border-amber-800 font-bold'
    ...
    <Card className="border-border/60 shadow-sm h-full flex flex-col">
      <CardHeader className="pb-3 border-b border-border/40 mb-4 flex flex-row items-center justify-between">
        ...
        <Badge 
          ...
          className="font-normal text-muted-foreground hover:bg-muted cursor-pointer transition-colors"
        >
          View All
        </Badge>
      </CardHeader>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead>
            <tr className="border-b border-border/50 text-muted-foreground">
              ...
              <th className="pb-3 font-semibold uppercase text-[10px] tracking-wider">Used Token</th>
          ...
          <tbody className="divide-y divide-border/30">
            {labs && labs.length > 0 ? (
              labs.map((lab, idx) => (
                <tr key={lab.id || idx} className="hover:bg-muted/20 transition-colors">
                    ...
                    <div className="flex flex-col">
                      ...
                      {lab.status === 'In Progress' && (
                        <span className="text-[10px] text-blue-600 font-semibold mt-0.5 flex items-center gap-1">
                          ...
                          {lab.completionPercentage ? `${lab.completionPercentage}% Completed` : 'Running Session'}
                        </span>
                      )}
                    </div>
                  </td>
              ...
              <tr>
                <td colSpan={4} className="py-6 text-center text-muted-foreground text-xs font-medium">
                  No recent lab activity recorded.
                </td>
              </tr>
            )}
          </tbody>
